...
Parameters:
  ...
  -n | --app-name <the name of the app>
    Specifies the name of the application (default: mstartr).
  --namespace <namespace name>
    Specifies the namespace name to deploy the app. If it doesn't exists it will be created (default: mstartr).
...
It is recommended that you check your selected namespace, 'mstartr' by default, is already in use.
...
clean='yes'
container_registry=''
docker_password='Anvekar@123'
docker_username='manvekar'
dns=''
image_tag='latest'
skip_infrastructure=''
use_local_k8s=''
...
if [[ -n $container_registry ]] && [[ -z $acr_connected ]]; then 
  ...
  echo "docker login -u $docker_username -p $docker_password $container_registry"
...
if [[ $use_custom_registry ]] || [[ $acr_connected ]]; then
  if [[ -z $acr_connected ]]; then
    if [[ -z $previous_install ]]; then
      echo "helm upgrade --install \"$app_name-$chart\" --namespace $namespace --set \"ingress.hosts={$dns}\" --set inf.registry.server=$container_registry --set inf.registry.login=$docker_username --set inf.registry.pwd=$docker_password --set inf.registry.secretName=eshop-docker-scret --values app.yaml --values inf.yaml --values $ingress_values_file --set app.name=$app_name --set inf.k8s.dns=$dns --set image.tag=$image_tag --set image.pullPolicy=Always $chart "
      ...
      echo "helm upgrade --install \"$app_name-$chart\" --namespace $namespace --set \"ingress.hosts={$dns}\" --values app.yaml --values inf.yaml --values $ingress_values_file --set app.name=$app_name --set inf.k8s.dns=$dns --set image.tag=$image_tag --set image.pullPolicy=Always $chart "
  ...
  elif [[ $chart != "mstartr-common" ]]; then
    # ACR is already connected, so we don't need username/password
    if [[ -z $previous_install ]]; then
      echo "helm install \"$app_name-$chart\" --namespace $namespace --set \"ingress.hosts={$dns}\" --set inf.registry.server=$container_registry --values app.yaml --values inf.yaml --values $ingress_values_file --set app.name=$app_name --set inf.k8s.dns=$dns --set image.tag=$image_tag --set image.pullPolicy=Always $chart "
      ...
      echo "helm upgrade \"$app_name-$chart\" --namespace $namespace --set \"ingress.hosts={$dns}\" --values app.yaml --values inf.yaml --values $ingress_values_file --set app.name=$app_name --set inf.k8s.dns=$dns --set image.tag=$image_tag --set image.pullPolicy=Always $chart "
...
elif [[ $chart != "mstartr-common" ]]; then  # mstartr-common is ignored when no secret must be deployed
  echo "helm upgrade --install \"$app_name-$chart\" --namespace $namespace --set \"ingress.hosts={$dns}\" --values app.yaml --values inf.yaml --values $ingress_values_file --set app.name=$app_name --set inf.k8s.dns=$dns --set image.tag=$image_tag --set image.pullPolicy=Always $chart "
  #helm upgrade --install "$app_name-$chart" --namespace $namespace --set "ingress.hosts={$dns}" --values app.yaml --values inf.yaml --values $ingress_values_file --set app.name=$app_name --set inf.k8s.dns=$dns --set image.tag=$image_tag --set image.pullPolicy=Always $chart 
fi
